...
/* Rough overlap between resume text and the job's title+description.
   Not a substitute for actually reading the posting -- but a fast way
   to stop scrolling past dozens of jobs a day you have no real shot at. */
...
function renderResultsList(filtered) {
  const resultsList = document.getElementById("resultsList");
  if (!filtered.length) {
    resultsList.innerHTML = '<div class="empty">No matches yet. Try broader keywords, add API keys in Settings for wider coverage, or check Sources for LinkedIn/Indeed manual-add.</div>';
    return;
  }
...
async function loadAutoDigest() {
  ...
  try {
    ...
    noteEl.textContent = "Auto-updated " + when + " -- " + filtered.length + " matches loaded below, sorted by resume match. Tap \"Search now\" anytime for a live refresh.";
    renderResultsList(filtered);
  } catch (e) {
    noteEl.textContent = 'No daily auto-digest set up yet (optional) -- see Settings for how, or just tap "Search now" for a live search.';
  }
}
/* ---------------- tracker ---------------- */
